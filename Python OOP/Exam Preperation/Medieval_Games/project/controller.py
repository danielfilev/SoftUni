...
class Controller:
    ...
    def sustain(self, player_name: str, sustenance_type: str):
        ...
        player.stamina = min(player.stamina + supply.energy,
                             Player.DEFAULT_STAMINA)
    ...
    def duel(self, first_player_name: str, second_player_name: str):
        ...
        error_msg = self.__get_error_msg(first_player, second_player)
        if error_msg:
            return error_msg
    ...
    def __attack(self, attacker, enemy):
        ...
        enemy.stamina = max(enemy.stamina -
                            attacker_dmg, 0)
        return enemy.stamina == 0

    def __get_error_msg(self, first_player, second_player):
        error_msg = ""
        if first_player.stamina == 0:
            error_msg += f"Player {first_player.name} does not have enough stamina."
